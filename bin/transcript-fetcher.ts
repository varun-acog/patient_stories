...
import { initializeDatabase, storeTranscript } from "../lib/database";
...
async function main() {
  ...
  try {
    ...
    if (options.videoId) {
      ...
      if (Array.isArray(inputData)) {
        if (inputData.length > 0 && typeof inputData[0] === 'string') {
          videoIds = inputData;
          safeLog("error", `Detected array of string IDs format`);
        } else if (inputData.length > 0 && typeof inputData[0] === 'object') {
          videoIds = inputData
            .map((item: any) => item.id || item.videoId)
            .filter((id: string | null): id is string => id !== null);
          safeLog("error", `Detected array of objects format`);
        }
      }
      
      safeLog("error", `Found ${videoIds.length} video IDs in input file`);
    }
    ...
    const outputData: { videoId: string; transcript: string }[] = [];
    for (const videoId of videoIds) {
      try {
        safeLog("error", `🔍 Fetching transcript for video ${videoId}...`);
        const transcript = await getTranscript(videoId);
        if (transcript) {
          outputData.push({ videoId, transcript });
          safeLog("error", `✅ Fetched transcript for video ${videoId}`);
        } else {
          safeLog("error", `⚠️ No transcript available for video ${videoId}`);
        }
      } catch (error) {
    ...
    // Store in database after processing
    for (const { videoId, transcript } of outputData) {
      try {
        await storeTranscript(videoId, transcript);
        safeLog("error", `✅ Stored transcript for video ${videoId} in database`);
      } catch (error) {
        safeLog("error", `❌ Error storing transcript for ${videoId} in database:`, error);
      }
    }

    if (options.outputFile) {
      await fs.writeFile(options.outputFile, JSON.stringify(outputData, null, 2));
      safeLog("error", `✅ Wrote transcript fetch results to ${options.outputFile}`);
    } else {
      for (const data of outputData) {
        try {
          process.stdout.write(JSON.stringify(data) + "\n");
          safeLog("error", `[DEBUG] Wrote to stdout: ${JSON.stringify({ videoId: data.videoId, transcriptLength: data.transcript.length })}`);
        } catch (error) {
          if (error.code !== 'EPIPE') {
            safeLog("error", `❌ Error writing to stdout: ${error.message}`);
          } else {
            safeLog("error", `[DEBUG] EPIPE detected, ignoring broken pipe`);
          }
        }
      }
    }

    safeLog("error", `✅ Transcript fetching completed for ${outputData.length}/${videoIds.length} videos`);
  } catch (error) {
