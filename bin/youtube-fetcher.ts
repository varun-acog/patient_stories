...
import { initializeDatabase, storeVideo } from "../lib/database";
...
async function main() {
  ...
  try {
    ...
    if (options.videoId) {
      ...
      safeLog("error", `Found ${videos.length} videos for "${options.disease}"`);
    }
    ...
    for (const video of videos) {
      try {
        ...
        safeLog("error", `✅ Processed metadata for video ${video.id}`);
      } catch (error) {
    ...
    for (const video of outputData) {
      try {
        await storeVideo(video);
        safeLog("error", `✅ Stored metadata for video ${video.id} in database`);
      } catch (error) {
        safeLog("error", `❌ Error storing video ${video.id} in database:`, error);
      }
    }
    ...
    if (options.outputFile) {
      ...
      safeLog("error", `✅ Wrote full video metadata to ${options.outputFile}`);
    }
    ...
    if (options.videoIdsFile) {
      ...
      safeLog("error", `✅ Wrote ${videoIds.length} video IDs to ${options.videoIdsFile}`);
    }
    ...
    if (!options.outputFile && !options.videoIdsFile) {
      for (const data of outputData) {
        try {
          ...
          safeLog("error", `[DEBUG] Wrote to stdout: ${JSON.stringify({ id: data.id, title: data.title })}`);
        } catch (error) {
          if (error.code !== 'EPIPE') {
