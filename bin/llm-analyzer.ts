...
import { initializeDatabase, storeAnalysis, storeVideo, getVideoMetadata } from "../lib/database";
...
async function main() {
  ...
  let inputData: { videoId: string; transcript: string; title?: string }[] = [];
  ...
  if (options.metadataFile) {
    ...
    safeLog("error", `[DEBUG] Loaded metadata for ${Object.keys(metadataMap).length} videos`);
  }
  ...
  for (const { videoId, transcript, title } of inputData) {
    try {
      safeLog("error", `🔍 Analyzing transcript for video ${videoId}...`);
      ...
      if (result) {
        ...
        await storeVideo({
          ...
          description: metadataMap[videoId]?.description || "",
          publishedDate: "",
          durationInSeconds: 0,
          viewCount: 0,
          url: `https://youtube.com/watch?v=${videoId}`,
        });
        await storeAnalysis(videoId, result);
        safeLog("error", `✅ Analysis completed and stored for ${videoId}`);
        outputData.push({ videoId, ...result });
      } else {
        safeLog("error", `⚠️ No analysis result for ${videoId}`);
      }
    } catch (error) {
  ...
  if (options.outputFile) {
    ...
    safeLog("error", `✅ Wrote analysis results to ${options.outputFile}`);
  } else {
    for (const data of outputData) {
      try {
        process.stdout.write(JSON.stringify(data) + "\n");
        safeLog("error", `[DEBUG] Wrote to stdout: ${JSON.stringify(data)}`);
      } catch (error) {
        if (error.code !== 'EPIPE') {
  ...
  safeLog("error", "✅ Analysis pipeline completed");
}
