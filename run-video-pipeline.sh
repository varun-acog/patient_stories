...
run_pipeline() {
  echo "Running full pipeline for '$SEARCH_TERM'..."
...
EOF
  ...
  docker compose run -T app npx tsx bin/youtube-fetcher.ts --disease "$SEARCH_TERM" --output-file "$METADATA_FILE" --video-ids-file "$VIDEOIDS_FILE" </dev/null || { echo "Error in youtube-fetcher.ts"; exit 1; }
...
if check_search_term; then
  echo "No need to run the pipeline; '$SEARCH_TERM' already exists in SearchConfig."
else
  run_pipeline
fi
